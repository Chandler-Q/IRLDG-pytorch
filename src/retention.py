...
class SimpleRetention(nn.Module):
    ...
    def reset_parameters(self):
        self.W_Q = init.xavier_uniform_(self.W_Q)
        self.W_K = init.xavier_uniform_(self.W_K)
        self.W_V = init.xavier_uniform_(self.W_V)
    ...
    def forward(self, X):
        ...
        Q = self.xpos(Q)
        K = self.xpos(K, downscale=True)    # downscale means the complex conjugate of theta in paper
    ...
    def forward_recurrent(self, x_n, s_n_1, n):
        ...
        s_n_1: (batch_size, hidden_size, v_dim) # S_(n-1)
        """
        ...
        s_n = (self.gamma * s_n_1).to(Q.device) + (K.transpose(-1, -2) @ V)
        
        return (Q @ s_n), s_n.detach()
